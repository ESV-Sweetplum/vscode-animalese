...
export async function handleKeyPress(context: vscode.ExtensionContext, event: vscode.TextDocumentChangeEvent) {
    ...
    source.detune.value = isHarmonic(key)
        ? 0
    ...
    if (settings.intonation_switchToExponentialFalloff) {
        ...
        gainNode.gain.linearRampToValueAtTime(0, audioContext.currentTime + settings.intonation_falloffTime);
    }
